...
double Parallel_Integration2(int n)
{
    ...
    #pragma omp parallel for ordered private(x) shared(h,sum) schedule(dynamic)
	for (i=1; i<=n; i++) {
        x = h * ((double)i - 0.5);
        double funcRes = f(x);
        #pragma omp ordered
...
double Parallel_Integration3(int n)
{
    ...
    double *funcResPointer = new double[n];

    #pragma omp parallel for private(x) shared(h, funcResPointer)
	for (i=1; i<=n; i++) {
		x = h * ((double)i - 0.5);
		funcResPointer[i-1] = f(x);
	}

    for (i=1; i<=n; i++) {
		sum += funcResPointer[i-1];
	}
...
double execute(double (*integr)(int), int n, double t_s)
{
    ...
    // omp_set_num_threads(8); // changes the number of threads
	t1 = getTime();
	res = integr(n);
	t_p = getTime() - t1;
