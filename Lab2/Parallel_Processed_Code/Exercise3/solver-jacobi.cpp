...
int solver(double **a, int n)
{
	...
	*/
	do {
		...
		#pragma omp parallel for collapse(2) private(h) reduction(max : diff)
		// By default, loop iteration counters in OpenMP loop constructs (in this case the i variable) in the for loop are set to private variables.
		// observation collapse makes j by default private, if we remove collapse then we need to set j as private.
		for (i=1; i<n-1; i++) {
			for (j=1; j<n-1; j++) {
		...
		*/
		...
		// By default, loop iteration counters in OpenMP loop constructs (in this case the i variable) in the for loop are set to private variables.
		// observation collapse makes the j also private by default, if we remove collapse j should be set private.
		for (i=1; i<n-1; i++) {
			for (j=1; j<n-1; j++) {
				a[i][j] = b[i][j];
			}
